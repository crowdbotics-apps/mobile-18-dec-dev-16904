...
function* api_v1_hgfjgjh_listWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_list, action)
    yield put(actions.api_v1_hgfjgjh_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_listFailed(err, action))
  }
}
function* api_v1_hgfjgjh_listWatcher() {
  yield takeEvery(types.API_V1_HGFJGJH_LIST, api_v1_hgfjgjh_listWorker)
}
function* api_v1_hgfjgjh_createWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_create, action)
    yield put(actions.api_v1_hgfjgjh_createSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_createFailed(err, action))
  }
}
function* api_v1_hgfjgjh_createWatcher() {
  yield takeEvery(types.API_V1_HGFJGJH_CREATE, api_v1_hgfjgjh_createWorker)
}
function* api_v1_hgfjgjh_readWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_read, action)
    yield put(actions.api_v1_hgfjgjh_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_readFailed(err, action))
  }
}
function* api_v1_hgfjgjh_readWatcher() {
  yield takeEvery(types.API_V1_HGFJGJH_READ, api_v1_hgfjgjh_readWorker)
}
function* api_v1_hgfjgjh_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_update, action)
    yield put(actions.api_v1_hgfjgjh_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_updateFailed(err, action))
  }
}
function* api_v1_hgfjgjh_updateWatcher() {
  yield takeEvery(types.API_V1_HGFJGJH_UPDATE, api_v1_hgfjgjh_updateWorker)
}
function* api_v1_hgfjgjh_partial_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_partial_update, action)
    yield put(actions.api_v1_hgfjgjh_partial_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_partial_updateFailed(err, action))
  }
}
function* api_v1_hgfjgjh_partial_updateWatcher() {
  yield takeEvery(
    types.API_V1_HGFJGJH_PARTIAL_UPDATE,
    api_v1_hgfjgjh_partial_updateWorker
  )
}
function* api_v1_hgfjgjh_deleteWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hgfjgjh_delete, action)
    yield put(actions.api_v1_hgfjgjh_deleteSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hgfjgjh_deleteFailed(err, action))
  }
}
function* api_v1_hgfjgjh_deleteWatcher() {
  yield takeEvery(types.API_V1_HGFJGJH_DELETE, api_v1_hgfjgjh_deleteWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    ...
    api_v1_hgfjgjh_listWatcher,
    api_v1_hgfjgjh_createWatcher,
    api_v1_hgfjgjh_readWatcher,
    api_v1_hgfjgjh_updateWatcher,
    api_v1_hgfjgjh_partial_updateWatcher,
    api_v1_hgfjgjh_deleteWatcher,
