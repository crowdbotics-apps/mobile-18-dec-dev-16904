...
const newConnector = axios.create({
  baseURL:
    "https://crowdbotics-slack-dev.herokuapp.com/dashboard/app/16904/storyboard/15898/",
  headers: { Accept: "application/json", "Content-Type": "application/json" }
})
const mobiledecAPI = axios.create({
  baseURL: "https://mobile-18-dec-dev-16904.botics.co/",
  headers: { Accept: "application/json", "Content-Type": "application/json" }
})
...
function api_v1_hgfjgjh_list(action) {
  return mobiledecAPI.get(`/api/v1/hgfjgjh/`)
}
function api_v1_hgfjgjh_create(action) {
  return mobiledecAPI.post(`/api/v1/hgfjgjh/`, null, { data: action.data })
}
function api_v1_hgfjgjh_read(action) {
  return mobiledecAPI.get(`/api/v1/hgfjgjh/${action.id}/`)
}
function api_v1_hgfjgjh_update(action) {
  return mobiledecAPI.put(`/api/v1/hgfjgjh/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_hgfjgjh_partial_update(action) {
  return mobiledecAPI.patch(`/api/v1/hgfjgjh/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_hgfjgjh_delete(action) {
  return mobiledecAPI.delete(`/api/v1/hgfjgjh/${action.id}/`)
}
...
export const apiService = {
  ...
  api_v1_hgfjgjh_list,
  api_v1_hgfjgjh_create,
  api_v1_hgfjgjh_read,
  api_v1_hgfjgjh_update,
  api_v1_hgfjgjh_partial_update,
  api_v1_hgfjgjh_delete,
